...
class modjy_servlet(HttpServlet, modjy_publisher, modjy_wsgi, modjy_impl):
    ...
    def process_param_container(self, param_container):
        ...
        while param_enum.hasMoreElements():
            ...
            self.do_param(
                param_name,
                param_container.getInitParameter(param_name))
    ...
    def init(self, delegator):
        ...
        self.exc_handler = getattr(
            modjy_exceptions,
            '%s_handler' %
            self.params['exc_handler'])()
    ...
    def service(self, req, resp):
        ...
        try:
            ...
            self.exc_handler.handle(
                req, resp, wsgi_environ, mx, (typ, value, tb))
    ...
    def dispatch_to_application(self, req, resp, environ):
        ...
        try:
            app_return = self.call_application(
                app_callable, environ, response_callable)
            if app_return is None:
                raise ReturnNotIterable(
                    "Application returned None: must return an iterable")
