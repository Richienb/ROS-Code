...
class modjy_impl:
    ...
    def deal_with_app_return(
            self,
            environ,
            start_response_callable,
            app_return):
        self.log.debug("Processing app return type: %s" %
                       str(type(app_return)))
        if isinstance(app_return, types.StringTypes):
            raise ReturnNotIterable(
                "Application returned object that was not an iterable: %s" % str(
                    type(app_return)))
            ...
            try:
                ...
                for next_piece in app_return:
                    if not isinstance(next_piece, types.StringTypes):
                        raise NonStringOutput(
                            "Application returned iterable containing non-strings: %s" % str(type(next_piece)))
                    if ix == 0:
                        # The application may have called start_response in the
                        # first iteration
                        if not start_response_callable.called:
                            raise StartResponseNotCalled(
                                "Start_response callable was never called.")
                        if not start_response_callable.content_length \
                            ...
                            start_response_callable.set_content_length(
                                len(next_piece))
                ...
                if expected_pieces != -1 and ix != expected_pieces:
                    raise WrongLength(
                        "Iterator len() was wrong. Expected %d pieces: got %d" %
                        (expected_pieces, ix))
    ...
    def add_classdirs(self, classdir_list):
        ...
        for cd in classdirs:
            self.log.info(
                "Adding directory %s to jython class file search path" %
                cd)
            sys.add_classdir(cd)
    ...
    def add_extdirs(self, extdir_list):
        ...
        for ed in extdirs:
            self.log.info(
                "Adding directory %s for .jars and .zips search path" %
                ed)
