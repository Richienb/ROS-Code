    ...
    def __call__(self, *args, **keywords):
        if len(args) != 1 or not isinstance(args[0], types.StringTypes):
            raise NonStringOutput(
                "Invocation of write callable requires exactly one string argument")
        try:
            # Jython implicitly converts the (binary) string to a byte array
            self.ostream.write(args[0])
