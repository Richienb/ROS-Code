...
class SerializableProxies(CustomMaker):
    ...
    # NOTE: SerializableProxies is itself a java proxy, but it's not a custom
    # one!
...
class PackageProxy(object):
    ...
    def __call__(
        ...
        if "." in pythonModuleName:
            # get around that will be called differently from regrtest, as
            # test.module instead of module
            pythonModuleName = pythonModuleName.split(".")[-1]
        return SerializableProxies(
            superclass,
            interfaces,
            className,
            pythonModuleName,
