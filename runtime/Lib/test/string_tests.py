...
class CommonTest(unittest.TestCase):
    ...
    def test_hash(self):
        # SF bug 1054139:  += optimization was not invalidating cached hash
        # value
    ...
    def test_split(self):
        ...
        self.checkequal(['a', '', 'b\x00c\x00d'],
                        'a\x00\x00b\x00c\x00d', 'split', '\x00', 2)
        ...
        self.checkequal(['a', 'b', 'c', 'd'], 'a//b//c//d',
                        'split', buffer('//'))
        self.checkequal(['a', 'b', 'c//d'], 'a//b//c//d',
                        'split', buffer('//'), 2)
    ...
    def test_rsplit(self):
        ...
        self.checkequal(['a\x00\x00b', 'c', 'd'],
                        'a\x00\x00b\x00c\x00d', 'rsplit', '\x00', 2)
        ...
        if test_support.is_jython:
            ...
            with memoryview('//') as target:
                self.checkequal(['a', 'b', 'c', 'd'],
                                'a//b//c//d', 'rsplit', target)
                self.checkequal(['a//b', 'c', 'd'],
                                'a//b//c//d', 'rsplit', target, 2)
    ...
    def test_strip(self):
        ...
        if test_support.is_jython and self.__class__.type2test in (
                str, bytearray):
            b = buffer('xyz')
    ...
    def test_replace(self):
        ...
        EQ("Th** ** a t**sue", "This is a tissue",
           "replace", "is", "**", sys.maxsize)
        ...
        if test_support.is_jython:
            for buftype in (buffer, memoryview, bytearray):
                ...
                EQ("Th** ** a t**sue", "This is a tissue",
                   "replace", buftype("is"), "**")
                EQ("cobobXcobocob", "bobobXbobobob",
                   "replace", buftype("bob"), "cob")
...
class MixinStrUnicodeUserStringTest:
    ...
    def test_startswith(self):
        ...
        if test_support.is_jython:
            for buftype in (buffer, memoryview, bytearray):
                ...
                self.checkequal(
                    True, 'hello', 'startswith', buftype('ello'), 1)
                self.checkequal(
                    True,
                    'helloworld',
                    'startswith',
                    ...
                    7)
                self.checkequal(
                    True, 'hello', 'startswith', buftype('he'), 0, -1)
                self.checkequal(
                    True, 'hello', 'startswith', buftype('ello'), -4)
                self.checkequal(True, 'hello', 'startswith', buftype('o'), -1)
                self.checkequal(
                    True, 'hello', 'startswith', (buftype('he'), 'ha'))
                self.checkequal(
                    True,
                    'helloworld',
                    'startswith',
    ...
    def test_formatting(self):
        ...
        for ordinal in (-100, 0x200000):
            ...
            self.checkraises((ValueError, OverflowError),
                             '%c', '__mod__', ordinal)

        longvalue = sys.maxsize + 10
        slongvalue = str(longvalue)
        if slongvalue[-1] in ("L", "l"):
            slongvalue = slongvalue[:-1]
        ...
        self.checkraises(TypeError, '%d', '__mod__', (42 + 0j)
                         )  # no int/long conversion provided
    ...
    def test_inplace_rewrites(self):
        # Check that strings don't copy and modify cached single-character
        # strings
    ...
    def test_partition(self):
        ...
        self.checkequal(('http', '://', 'www.python.org'),
                        S, 'partition', '://')
...
class MixinStrUnicodeTest:
    ...
    def test_bug1001011(self):
        ...
        if t is unicode:
            ...
            s2 = u"".join([s1])
            self.assertTrue(s1 is not s2)
            self.assertTrue(isinstance(s2, unicode))  # promotes!
