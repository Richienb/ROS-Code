...
class CPickleTestCase(unittest.TestCase):
    ...
    def test_zero_long(self):
        self.assertEqual(cPickle.loads(cPickle.dumps(0, 2)), 0)
        self.assertEqual(cPickle.dumps(0, 2), pickle.dumps(0, 2))
    ...
    def test_find_global(self):
        ...
        class A(object):
            ...
            def __eq__(self, other):
                if isinstance(
                        other,
                        A) and self.x == other.x and self.y == other.y:
                    return True
                return False
        ...
        def restrictive_find_global(module, clsname):
            if clsname == 'A':
                ...
                raise pickle.UnpicklingError(
                    "Cannot load class", module, clsname)
    ...
    def testWithUserDefinedImport(self):
        ...
        try:
            if "no_such_module" in sys.modules:
                # force cPickle to call __import__
                del sys.modules["no_such_module"]
            self.assertRaises(
                ImportError,
                cPickle.loads,
                pickle.GLOBAL +
