...
class CollectionProxyTest(unittest.TestCase):
    ...
    def check_list(
        ...
        for result, type_name in zip(results, list_type_names):
            try:
                ...
                print result
            self.assertEquals(
                len(control), len(result), "%s: length for %s does not match that of list" %
                (test_name, type_name))
            for control_value, result_value in zip(control, result):
                self.assertEquals(
                    control_value,
                    result_value,
                    "%s: values from %s do not match those from list" %
                    (test_name,
                     type_name))
    ...
    def _list_op_test(self, initial_value, op_func, test_name):
        ...
        if not isinstance(results[0], list):
            for r, n in zip(results[1:], list_type_names[1:]):
                self.assertEquals(
                    results[0], r, '%s: result for list does not match result for java type %s' %
                    (test_name, n))
        else:
            self.check_list(results[0],
                            results[1:],
                            list_type_names[1:],
                            initial_value,
                            test_name)
    ...
    def test_get_integer(self):
        ...
        for i in xrange(-7, 7):
            self._list_op_test(
                initial_value, lambda xs: xs[i], 'get_integer [%d]' %
                (i,))
        ...
        for i in [None] + range(-7, 7):
            for j in [None] + range(-7, 7):
                for k in [None] + range(-7, 7):
                    self._list_op_test(
                        initial_value, lambda xs: xs[i:j:k], 'get_slice [%s:%s:%s]' % (i, j, k))
    ...
    def test_set_integer(self):
        ...
        for i in xrange(-7, 7):
            self._list_op_test(
                initial_value, make_op_func(i), 'set_integer [%d]' %
                (i,))
    ...
    def test_set_slice(self):
        ...
        for i in [None] + range(-7, 7):
            for j in [None] + range(-7, 7):
                for k in [None] + range(-7, 7):
                    self._list_op_test(
                        initial_value, make_op_func(
                            i, j, k, []), 'set_slice [%s:%s:%s]=[]' %
                        (i, j, k))
                    self._list_op_test(
                        initial_value, make_op_func(
                            i, j, k, range(
                                0, 2)), 'set_slice [%s:%s:%s]=range(0,2)' %
                        (i, j, k))
                    self._list_op_test(
                        initial_value, make_op_func(
                            i, j, k, range(
                                0, 4)), 'set_slice [%s:%s:%s]=range(0,4)' %
                        (i, j, k))
                    self._list_op_test(
                        initial_value, make_op_func(
                            i, j, k, xrange(
                                0, 2)), 'set_slice [%s:%s:%s]=xrange(0,2)' %
                        (i, j, k))
                    self._list_op_test(
                        initial_value, make_op_func(
                            i, j, k, self._arraylist_of(
                                range(
                                    0, 2))), 'set_slice [%s:%s:%s]=ArrayList(range(0,2))' %
                        (i, j, k))

        self._list_op_test([1, 2, 3, 4, 5], make_op_func(
            1, None, None, [1, 2, 3, 4, 5]), 'set_slice [1:]=[1,2,3,4,5]')
    ...
    def test_del_integer(self):
        ...
        for i in xrange(-7, 7):
            self._list_op_test(
                initial_value, make_op_func(i), 'del_integer [%d]' %
                (i,))
    ...
    def test_override_getsetdelitem(self):
        # Create an ArrayList subclass that provides some silly overrides for
        # get/set/del item
        class MyList (ArrayList):
        ...
        for i in [None] + range(-7, 7):
            for j in [None] + range(-7, 7):
                for k in [None] + range(-7, 7):
                    ...
                    resultPy = self._perform_op(
                        destPy, make_op_func(i, j, k, sourcePy))
                    resultJUL = self._perform_op(
                        destJUL, make_op_func(i, j, k, sourceJUL))
                    resultIter = self._perform_op(
                        destIter, make_op_func(i, j, k, sourceIter))
