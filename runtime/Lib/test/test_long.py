...
class LongTest(unittest.TestCase):
    ...
    def check_division(self, x, y):
        ...
        if y > 0:
            self.assertTrue(
                0 <= r < y, Frm(
                    "bad mod from divmod on %r and %r", x, y))
        else:
            self.assertTrue(
                y < r <= 0, Frm(
                    "bad mod from divmod on %r and %r", x, y))
    ...
    def test_karatsuba(self):
        ...
        for abits in bits:
            ...
            for bbits in bits:
                ...
                self.assertEqual(
                    x, y, Frm(
                        "bad result for a*b: a=%r, b=%r, x=%r, y=%r", a, b, x, y))
    ...
    def check_bitop_identities_3(self, x, y, z):
        ...
        eq((x ^ y) ^ z, x ^ (y ^ z),
           Frm("(x ^ y) ^ z != x ^ (y ^ z) for x=%r, y=%r, z=%r", (x, y, z)))
        eq(x & (y | z), (x & y) | (x & z), Frm(
            "x & (y | z) != (x & y) | (x & z) for x=%r, y=%r, z=%r", (x, y, z)))
        eq(x | (y & z), (x | y) & (x | z), Frm(
            "x | (y & z) != (x | y) & (x | z) for x=%r, y=%r, z=%r", (x, y, z)))
    ...
    def test_bitop_identities(self):
        ...
        for lenx in digits:
            ...
            for leny in digits:
                ...
                self.check_bitop_identities_3(
                    x, y, self.getran((lenx + leny) // 2))
    ...
    def test_long(self):
        self.assertEqual(long(314), 314)
        self.assertEqual(long(3.14), 3)
        self.assertEqual(long(314), 314)
        ...
        self.assertEqual(long(-3.14), -3)
        self.assertEqual(long(3.9), 3)
        self.assertEqual(long(-3.9), -3)
        self.assertEqual(long(3.5), 3)
        self.assertEqual(long(-3.5), -3)
        self.assertEqual(long("-3"), -3)
        self.assertEqual(long("0b10", 2), 2)
        self.assertEqual(long("0o10", 8), 8)
        self.assertEqual(long("0x10", 16), 16)
        if test_support.have_unicode:
            self.assertEqual(long(unicode("-3")), -3)
        # Different base:
        self.assertEqual(long("10", 16), 16)
        if test_support.have_unicode:
            self.assertEqual(long(unicode("10"), 16), 16)
        # Check conversions from string (same test set as for int(), and then
        # some)
        LL = [
            ('1' + '0' * 20, 10**20),
            ('1' + '0' * 100, 10**100)
    ...
    def test_conversion(self):
        ...
        self.assertEqual(long(Foo0()), 42)
        self.assertEqual(long(Foo1()), 42)
        self.assertEqual(long(Foo2()), 42)
    ...
    def test_misc(self):
        ...
        try:
            self.assertEqual(
                x, hugepos, "converting sys.maxint to long and back to int fails")
        ...
        try:
            self.assertEqual(
                x, hugeneg, "converting -sys.maxint-1 to long and back to int fails")
        except OverflowError:
...
# ----------------------------------- tests of auto int->long conversion
    ...
    def test_auto_overflow(self):
        ...
        def checkit(*args):
            ...
            self.assertEqual(
                got, expected, Frm(
                    "for %r expected %r got %r", args, expected, got))
                    ...
                    for z in special:
                        if z != 0:
                            if y >= 0:
                                ...
                                self.assertRaises(
                                    TypeError, pow, longx, longy, long(z))
    ...
    def test_float_conversion(self):
        ...
        for x, y in [(1, 0), (2, 2), (3, 4), (4, 4), (5, 4), (6, 6), (7, 8)]:
            for p in xrange(15):
                self.assertEqual(
                    long(float(2**p * (2**53 + x))), 2**p * (2**53 + y))
        ...
        for x, y in [(0, 0), (1, 0), (2, 0), (3, 4), (4, 4), (5, 4), (6, 8),
            ...
            for p in xrange(15):
                self.assertEqual(
                    long(float(2**p * (2**54 + x))), 2**p * (2**54 + y))
