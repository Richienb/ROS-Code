...
class AsynchronousServer:
    ...
    def select_acceptable(self):
        return select.select([self.server_socket], [
                             self.server_socket], [], SELECT_TIMEOUT)[0]
            ...
            try:
                if self.select_writable():
                    ...
                    if test_support.is_jython and total_bytes > 10000:
                        # Netty will buffer indefinitely, so just pick an
                        # arbitrary cutoff
                        return total_bytes
                else:
                    return total_bytes
...
class AsynchronousClient(AsynchronousHandler):
    ...
    def start_connect(self):
        ...
        if result == errno.EISCONN:
            ...
            assert result in [errno.EINPROGRESS, errno.ENOTCONN], "connect_ex returned %s (%s)" % (
                result, errno.errorcode.get(result, "Unknown errno"))
