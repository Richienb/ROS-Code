        ...
        def f(*args, **kwargs):
            try:
                ...
                if (ssl.OPENSSL_VERSION_INFO == (0, 9, 8, 15, 15) and
                    platform.linux_distribution() == ('debian', 'squeeze/sid', '') and
                        'Invalid SSL protocol variant specified' in str(e)):
...
class BasicSocketTests(unittest.TestCase):
    ...
    def test_openssl_version(self):
        ...
        self.assertTrue(
            s.startswith(
                "OpenSSL {:d}.{:d}.{:d}".format(
                    major, minor, fix)), (s, t))

    def test_ciphers(self):
        if not test_support.is_resource_enabled(
                'network') or test_support.is_jython:
        ...
        with test_support.transient_internet("svn.python.org"):
            ...
            try:
                pem = ssl.get_server_certificate(
                    ("svn.python.org", 443), ca_certs=CERTFILE)
            except ssl.SSLError:
                # should fail
                pass
            else:
            ...
            pem = ssl.get_server_certificate(
                ("svn.python.org", 443), ca_certs=SVN_PYTHON_ORG_ROOT_CERT)
            if not pem:
                self.fail("No server certificate on svn.python.org:443!")
            if test_support.verbose:
                sys.stdout.write(
...
try:
                    ...
                    try:
                        ...
                        if not msg:
                            ...
                            if (test_support.verbose and
                                ...
                                ctype = (
                                    self.sslconn and "encrypted") or "unencrypted"
                                sys.stdout.write(
                                    " server: read %s (%s), sending back %s (%s)...\n" %
                                    (repr(msg), ctype, repr(
                                        msg.lower()), ctype))
        ...
        def __init__(
            ...
            if wrap_accepting_socket:
                ...
                if test_support.verbose and self.chatty:
                    sys.stdout.write(
                        ' server:  wrapped server socket as %s\n' % str(
                            self.sock))
            ...
            class ConnectionHandler(asyncore.dispatcher_with_send):
                ...
                def __init__(self, conn, certfile):
                    ...
                    self.socket = ssl.wrap_socket(
                        conn, server_side=True, certfile=certfile, do_handshake_on_connect=False)
    ...
    class SocketServerHTTPSServer(threading.Thread):
        ...
        class HTTPSServer(HTTPServer):
            ...
            def __init__(self, server_address, RequestHandlerClass, certfile):
                ...
                # we assume the certfile contains both private key and
                # certificate
                self.certfile = certfile
    ...
    class ThreadedTests(unittest.TestCase):
        ...
        def test_getpeercert(self):
            ...
            with server:
                ...
                if test_support.verbose:
                    ...
                    sys.stdout.write(
                        "Connection cipher is " + str(cipher) + '.\n')
        ...
        def test_protocol_sslv3(self):
            ...
            if hasattr(ssl, 'PROTOCOL_SSLv2'):
                try_protocol_combo(
                    ssl.PROTOCOL_SSLv3, ssl.PROTOCOL_SSLv2, False)
        ...
        def test_protocol_tlsv1(self):
            ...
            if hasattr(ssl, 'PROTOCOL_SSLv2'):
                try_protocol_combo(
                    ssl.PROTOCOL_TLSv1, ssl.PROTOCOL_SSLv2, False)
...
def test_main(verbose=False):
    ...
    if test_support.is_resource_enabled(
            'network') and not test_support.is_jython:
