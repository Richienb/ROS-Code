...
class dbextsTestCase(runner.SQLTestCase):
    ...
    def setUp(self):
        ...
        template = template % (
            args["url"], args["usr"], args["pwd"], args["driver"])
    ...
    def testBulkcopy(self):
        ...
        assert len(bcp.columns) == 1, "one column should be specified, [%d] found" % (
            len(bcp.columns))

        bcp = self.db.bulkcopy(
            "dbexts_test", "two", include=[
                'a', 'b', 'c'], exclude=['a'])
        assert len(bcp.columns) == 2, "expected two columns, found [%d]" % (
            len(bcp.columns))
        ...
        assert done == 3, "expecting three rows to be handled but not inserted, found [%d]" % (
            done)

        bcp = self.db.bulkcopy("dbexts_test", "two", include=['a'])
        done = bcp.transfer(self.db)
        assert done == 3, "expecting three rows to be inserted, found [%d]" % (
            done)
        ...
        assert done == 4, "expecting four rows to be inserted, found [%d]" % (
            done)
        bcp.rowxfer([300])
        bcp.rowxfer([401])
        bcp.rowxfer([502])
        ...
        assert done == 2, "expecting two rows to be inserted, found [%d]" % (
            done)
        assert bcp.total == 10, "expecting 10 rows to be inserted, found [%d]" % (
            bcp.total)

        bcp = self.db.bulkcopy("dbexts_test", "two", include=['a'])
        done = bcp.transfer(self.db)
        assert done == 16, "expecting sixteen rows to be inserted, found [%d]" % (
            done)
    ...
    def testResultSetWrapper(self):
        ...
        self._insertInto("two", 30)
        h, r = self.db.raw(
            "select * from two where a in (?, ?, ?, ?) order by a", [(12, 15, 17, 8)])
        ...
        assert rs[0]['A'] == 8, "expected [8], got [%s]" % (rs[0]['A'])
        assert len(rs[0]['b':]) == 2, "expected [2], got [%s]" % (
            len(rs[0]['b':]))
        assert len(rs[0]['a':'b']) == 1, "expected [1], got [%s]" % (
            len(rs[0]['a':'b']))
    ...
    def testMultipleResultSetConcatentation(self):
        ...
        self._insertInto("two", 30)
        # first is non None
        h, r = self.db.raw("select * from two where a = ?",
                           [(12,), (8001,), (15,), (17,), (8,), (9001,)])
        assert len(r) == 4, "expected [4], got [%d]" % (len(r))
        # first is None
        h, r = self.db.raw("select * from two where a = ?",
                           [(1200,), (8001,), (15,), (17,), (8,), (9001,)])
    ...
    def testBulkcopyWithDynamicColumns(self):
        ...
        assert done == 32, "expecting thirty two rows to be inserted, found [%d]" % (
            done)
        ...
        for u in (0, 1):
            ...
            for v in (0, 1):
                ...
                try:
                    ...
                    self.fail(
                        "failed autocommit query with u=[%d], v=[%d]" %
                        (u, v))
