...
class zxJDBCTestCase(zxCoreTestCase):
    ...
    def setUp(self):
        ...
        try:
            c.execute(
                "create table zxtesting (id int not null, name varchar(32), state varchar(32), primary key (id))")
            self.db.commit()
            c.execute(
                "insert into zxtesting (id, name, state) values (1, 'test0', 'il')")
            c.execute(
                "insert into zxtesting (id, name, state) values (2, 'test1', 'wi')")
            c.execute(
                "insert into zxtesting (id, name, state) values (3, 'test2', 'tx')")
            c.execute(
                "insert into zxtesting (id, name, state) values (4, 'test3', 'co')")
            c.execute(
                "insert into zxtesting (id, name, state) values (5, 'test4', 'il')")
            c.execute(
                "insert into zxtesting (id, name, state) values (6, 'test5', 'ca')")
            c.execute(
                "insert into zxtesting (id, name, state) values (7, 'test6', 'wi')")
            self.db.commit()
        finally:
            c.close()
...
class zxAPITestCase(zxJDBCTestCase):
    ...
    def testCursorKeywords(self):
        ...
        self._test_cursorkeywords(
            1,
            zxJDBC.TYPE_SCROLL_INSENSITIVE,
            zxJDBC.CONCUR_READ_ONLY)
        self.assertRaises(
            TypeError,
            self.cursor,
            1,
    ...
    def testClosingCursor(self):
        ...
        try:
            ...
            c.close()
        self.assertRaises(zxJDBC.ProgrammingError, c.execute,
                          ("select * from zxtesting",))
    ...
    def testClosingConnectionWithOpenCursors(self):
        ...
        self.assertRaises(zxJDBC.ProgrammingError, c.execute,
                          ("select * from zxtesting",))
        self.assertRaises(zxJDBC.ProgrammingError, d.execute,
                          ("select * from zxtesting",))
        self.assertRaises(zxJDBC.ProgrammingError, e.execute,
                          ("select * from zxtesting",))
    ...
    def _test_scrolling(self, dynamic=0):
        ...
        try:
            ...
            self.assertRaises(
                zxJDBC.ProgrammingError,
                c.scroll,
                1,
                "somethingelsealltogether")
        finally:
            c.close()
    ...
    def _test_rownumber(self, dynamic=0):
        ...
        try:
            if not dynamic:
                ...
                # maybe the 'possibility' of rows should change .rownumber to
                # 0?
    ...
    def _test_rowcount(self, dynamic=0):
        ...
        try:
            ...
            c.next()
            if dynamic:
                # dynamic cursors only know about the number of rows
                # encountered
                self.assertEquals(3, c.rowcount)
            else:
    ...
    def testTupleParams(self):
        ...
        try:
            self.assertRaises(
                zxJDBC.ProgrammingError,
                c.execute,
                "select * from zxtesting where id = ?",
                params=4)
    ...
    def testConnectionAttribute(self):
        ...
        try:
            ...
            assert isinstance(
                c.connection, PyConnection), "expected PyConnection"
            self.assertRaises(
                TypeError, setattr, (c, "connection", None), None)
        finally:
            c.close()
    ...
    def testQueryWithParameter(self):
        ...
        try:
            c.execute(
                "select name from zxtesting where state = ?", [
                    ("il",)], {
                    0: zxJDBC.VARCHAR})
            data = c.fetchall()
    ...
    def testDate(self):
        ...
        v = zxJDBC.Date(
            ...
            c.get(
                Calendar.DATE))
        assert o.equals(v), "incorrect date conversion using java, got [%ld], expected [%ld]" % (
            v.getTime(), o.getTime())
        ...
        assert o.equals(v), "incorrect date conversion, got [%ld], expected [%ld]" % (
            v.getTime(), o.getTime())

    def testTime(self):
        """testing creation of Time"""

        # Java uses milliseconds and Python uses seconds, so adjust the time
        # accordingly
    ...
    def _test_precision(self, xxx_todo_changeme, diff, values, attr):
        ...
        try:
            ...
            for i in range(0, len(f)):
                ...
                if attr:
                    v = getattr(v, attr)()
                msg = "expected [%0.10f], got [%0.10f] for index [%d] of [%d]" % (
                    v, f[i][1], (i + 1), len(f))
    ...
    def testBigDecimal(self):
        ...
        values = [BigDecimal(x).setScale(2, BigDecimal.ROUND_UP)
                  for x in [4.22, 123.44, 292.09, 33.2, 102.00, 445]]
        self._test_precision(
            self.table("floattable"),
            lambda x,
            y,
            b=BigDecimal: b(x).subtract(y).doubleValue(),
            values,
            "doubleValue")
    ...
    def testUpdateCount(self):
        ...
        try:
            c.execute(
                "insert into zxtesting values (?, ?, ?)", [
                    (500, 'bz', 'or')])
            assert c.updatecount == 1, "expected [1], got [%d]" % (
                c.updatecount)
            c.execute("select * from zxtesting")
            ...
            assert c.updatecount == 8, "expected [8], got [%d]" % (
                c.updatecount)
    ...
    def _test_time(
        ...
        try:
            c.execute(sql)
            dates = map(lambda x, f=factory: f(*x), values)
            for a in dates:
                c.execute(
            ...
            assert _cmp(f[0][1], dates[0]) == 0, "expected date [%s], got [%s]" % (
                str(dates[0]), str(f[0][1]))

            c.execute(
                "delete from %s where b = ?" %
                (tabname), [
                    (dates[1],)], {
                    0: _type})
            self.db.commit()
    ...
    def testUpdateSelectByTimestamp(self):
        ...
        assert self.has_table(
            "timestamptable"), "missing attribute timestamptable"
        ...
        def _cmp_(x, y):
            ...
            yt = (
                ...
                y.getSeconds())
            return not xt == yt
        values = [(1996, 6, 22, 10, 11, 12), (2000, 11,
                                              12, 3, 1, 12), (2001, 1, 12, 4, 9, 24)]
        self._test_time(
    ...
    def testOrderOfArgs(self):
        ...
        try:
            ...
            c.execute("select * from zxtesting where id = ?",
                      bindings={0: zxJDBC.INTEGER}, params=[(3,)])
            f = c.fetchall()
            assert len(f) == 1, "expected length [1], got [%d]" % (len(f))
            ...
            c.execute(
                "select * from zxtesting where id = 3",
                bindings={},
                params=[])
            f = c.fetchall()
            assert len(f) == 1, "expected length [1], got [%d]" % (len(f))
    ...
    def _test_fetching(self, dynamic=0):
        ...
        try:
            ...
            c.execute("select * from zxtesting")
            f = c.fetchmany()
            assert len(f) == c.arraysize, "expecting [%d] rows, got [%d]" % (
                c.arraysize, len(f))
            c.execute("select * from zxtesting")
            c.arraysize = 4
            f = c.fetchmany()
    ...
    def testFetchingBeforeExecute(self):
        ...
        try:
            try:
                ...
                self.fail(
                    "excepted exception calling fetchall() prior to execute()")
        finally:
            c.close()
    ...
    def testBindingsWithNoParams(self):
        ...
        try:
            self.assertRaises(zxJDBC.ProgrammingError, c.execute,
                              "select * from zxtesting", {0: zxJDBC.INTEGER})

            # test an inappropriate value for a binding
            self.assertRaises(zxJDBC.ProgrammingError, c.execute,
                              "select * from zxtesting", {0: {}})
        finally:
            c.close()
    ...
    def testRowid(self):
        ...
        c = self.cursor()
        assert c.lastrowid is None, "expected initial lastrowid to be None"
...
class LOBTestCase(zxJDBCTestCase):
    ...
    def _test_blob(self, obj=0):
        ...
        try:
            ...
            c.execute("insert into %s (a, b) values (?, ?)" %
                      (tabname), [(0, fp)], {1: zxJDBC.BLOB})
            self.db.commit()
...
class BCPTestCase(zxJDBCTestCase):
    ...
    def testCSVPipe(self):
        ...
        try:
            ...
            try:
                c.execute(
                    "insert into zxtesting (id, name, state) values (?, ?, ?)", [
                        (1000, 'this,has,a,comma', 'and a " quote')])
                c.execute("insert into zxtesting (id, name, state) values (?, ?, ?)", [
                          (1001, 'this,has,a,comma and a "', 'and a " quote')])
                # ORACLE has a problem calling stmt.setObject(index, null)
                c.execute("insert into zxtesting (id, name, state) values (?, ?, ?)", [
                          (1010, '"this,has,a,comma"', None)], {2: zxJDBC.VARCHAR})
                self.db.commit()
            finally:
                self.db.rollback()
                c.close()
    ...
    def testDBPipe(self):
        ...
        try:
            ...
            c = self.cursor()
            c.execute(
                "create table zxtestingbcp (id int not null, name varchar(20), state varchar(2), primary key (id))")
            self.db.commit()
            ...
            dbSource = DBSource(
                ...
                None,
                None)
            dbSink = DBSink(dst, c.datahandler.__class__,
                            "zxtestingbcp", None, None, 1)
            ...
            assert one == two, "expected [%d] rows in destination, got [%d] (sql)" % (
                one, two)
            assert one == cnt, "expected [%d] rows in destination, got [%d] (bcp)" % (
                one, cnt)
