...
class ZipExtFile(io.BufferedIOBase):
    ...
    def read1(self, n):
        ...
        # Read from file.
        if self._compress_left > 0 and n > len_readbuffer + \
                len(self._unconsumed):
...
class ZipFile(object):
    ...
    def _extract_member(self, member, targetpath, pwd):
        ...
        arcname = os.path.sep.join(
            x for x in arcname.split(
                os.path.sep) if x not in (
                '',
                os.path.curdir,
                os.path.pardir))
        if os.path.sep == '\\':
    ...
    def close(self):
        ...
        try:
            if self.mode in (
                    "w", "a") and self._didModify:  # write ending records
                count = 0
                pos1 = self.fp.tell()
                ...
                endrec = struct.pack(structEndArchive,
                                     stringEndArchive,
                                     0,
                                     0,
                                     centDirCount,
